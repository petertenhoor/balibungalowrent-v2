	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.','booklium'  );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							esc_html__( 'Your license key expired on %s.' ,'booklium'),
					...
					case 'revoked' :
						...
						$message = esc_html__( 'Your license key has been disabled.' ,'booklium' );
						break;

					case 'missing' :

						$message = esc_html__( 'Invalid license.','booklium'  );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = esc_html__( 'Your license is not active for this URL.' ,'booklium' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.','booklium'  ), $args['name'] );
						break;

					case 'no_activations_left':

						$message = esc_html__( 'Your license key has reached its activation limit.','booklium'  );
						break;

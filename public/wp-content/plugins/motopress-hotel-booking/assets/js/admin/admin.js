...
MPHBAdmin.ExportBookings = can.Control.extend(
	{},
	{
		...
		exportImports: null, // input[type="checkbox"]
		...
		initElements: function (root) {
			...
			this.exportImports = root.find('.mphb-export-imports');
		...
		getValues: function () {
			...
			values['export_imports'] = this.exportImports.prop('checked');
...
MPHBAdmin.get_query_arg = function (argName) {
...
MPHBAdmin.Ctrl = can.Control.extend({
	...
	renderValue: function (control) {
		return MPHBAdmin.Ctrl.getValue(control);
	}
}, {
	parentForm: null,
	init: function (el, args) {
...
MPHBAdmin.AmountCtrl = MPHBAdmin.Ctrl.extend({
	...
	getValue: function (control) {
		...
		if (inputs.length == 1) {
			return parseInt(inputs.val());
		} else {
			return {
				adults:   parseInt(inputs[0].value),
				children: parseInt(inputs[1].value)
			};
		}
	},
	renderValue: function (control) {
...
MPHBAdmin.ComplexCtrl = MPHBAdmin.Ctrl.extend({}, {
	...
	initAddBtns: function () {
		var self = this;
		this.element.on('click', '.mphb-complex-add-item[data-id="' + this.uniqid + '"]', function (e) {
			e.preventDefault();

			var position = e.target.classList.contains('mphb-top-button') ? 'top' : 'bottom';
	...
	initMoveBtns: function () {
		var selectsSelector = '.mphb-move-select[data-id="' + this.uniqid + '"]';
		var self = this;

		this.itemsHolder.on('change', selectsSelector, function (e) {
			var $item = $(this).closest(self.itemSelector);
			var $select = $(e.target);
			var direction = $select.val();
	...
	setKeys: function (wrappers) {
		...
		wrappers.each(function (index, wrapper) {
			...
			$wrapper.find('[data-full-name*="[%key_' + self.uniqid + '%]"]').each(function () {
				var $element = $(this);
				var fullName = $element.attr('data-full-name').replace(keyRegEx, key);
				$element.attr('data-full-name', fullName);
			});
	...
	addItemToHolder: function (item, position) {
		if (position === 'top') {
			this.itemsHolder.prepend(item);
		} else {
	...
	addItem: function (position) {
		...
		this.addItemToHolder(item, position);
		var ctrls = item.find('.mphb-ctrl:not([data-inited])');
		MPHBAdmin.Plugin.myThis.setControls(ctrls);
	},
	moveItem: function ($item, direction) {
		switch (direction) {
			case 'up':     $item.insertBefore($item.prev()); break;
			case 'down':   $item.insertAfter($item.next());  break;
			case 'top':    $item.insertBefore(this.itemsHolder.children(this.itemSelector).first()); break;
			case 'bottom': $item.insertAfter(this.itemsHolder.children(this.itemSelector).last());   break;
		}
	}
});
...
MPHBAdmin.DatePickerCtrl = MPHBAdmin.Ctrl.extend({
	...
	renderValue: function (control) {
		return MPHBAdmin.DatePickerCtrl.getValue(control);
	}
}, {
	input: null,
	hiddenInput: null,
...
MPHBAdmin.RulesListCtrl = MPHBAdmin.Ctrl.extend({}, {
	editClass: 'mphb-list-item-editing',
	editText: '',
	doneText: '',
	lastIndex: -1,
	rulesCount: 0,
	...
	editingRule: null,
	noRulesMessage: null,
	prependNewItems: true, // ... instead of append
	init: function (element, args) {
		this._super(element, args);
		...
		this.noRulesMessage = element.find('.mphb-empty-message');
		...
		var prototypeElement = this.tbody.children('.mphb-list-item-prototype');
		var rulePrototype = prototypeElement.clone();

		prototypeElement.remove();

		rulePrototype.removeClass('mphb-list-item-prototype mphb-hide');
	...
	".mphb-add-button click": function () {
		this.addRule();
	},
	".mphb-edit-button click": function (button, event) {
		var rule = this.getRuleByButton(button);
		this.toggleEdit(rule);
	},
	".mphb-delete-button click": function (button, event) {
		var rule = this.getRuleByButton(button);
	...
	toggleEdit: function (rule) {
		if (this.editingRule != null) {
			...
			// minimized
			this.ruleDone(this.editingRule);

			// Change text from "Done" to "Edit"
			this.editingRule.find('.mphb-edit-button').text(this.editText);
		}

		if (this.isEditingRule(rule)) {
			this.editingRule = null; // Already removed the class in ruleDone()
		} else {
			this.editingRule = rule;
			rule.addClass(this.editClass);

			// Change text from "Edit" to "Done"
			rule.find('.mphb-edit-button').text(this.doneText);
		}
	},
	/**
	 * @since 5.0.0
	 *
	 * @param {Object} rule jQuery object.
	 */
	ruleDone: function (rule) {
		this.renderValues(rule);
		rule.removeClass(this.editClass);
	},
	/**
	 * @param {Object} rule &lt;tr&gt; element.
	 */
	...
	renderValues: function (rule) {
		...
		rule.children('td').each(function () {
			...
			td.children('.mphb-rendered-value').html(result);
		});
	},
	/**
	...
	getValues: function (rule) {
		...
		rule.children('td').each(function () {
			...
			if (control.length != 0) {
				var name = control.attr('data-initial-name');
				var value = self.getValue(control);
	...
	/**
	 * Tested the result with only textarea field.
	 *
	 * @since 5.0.0
	 *
	 * @param {Object} control jQuery object.
	...
	getValue: function (control) {
		...
		switch (type) {
			case 'text':
				result = MPHBAdmin.Ctrl.getValue(control);
				break;

			case 'datepicker':
				result = MPHBAdmin.DatePickerCtrl.getValue(control);
				break;
			...
			case 'number':
				result = MPHBAdmin.NumberCtrl.getValue(control);
				break;

			case 'select':
			case 'dynamic-select':
				var value = control.children('select').val();
				result = (value != undefined) ? value : '';
				break;

			case 'single-checkbox':
				result = MPHBAdmin.SingleCheckboxCtrl.getValue(control);
				break;

			case 'multiple-checkbox':
				result = MPHBAdmin.MultipleCheckboxCtrl.getValue(control);
				break;

			case 'amount':
				result = MPHBAdmin.AmountCtrl.getValue(control);
				break;
		}
...
MPHBAdmin.NotesListCtrl = MPHBAdmin.RulesListCtrl.extend({}, {
	...
	init: function (element, args) {
		...
		this.bookingId = parseInt(MPHBAdmin.get_query_arg('post'));
		...
		rules.each(function () {
			var rule = $(this);
			var index = parseInt(rule.attr('data-id'));

			self.notes[index] = self.getValues(rule);
		});
	},
	/**
	 * @since 5.0.0
	...
	ruleDone: function (rule) {
		...
		// Save note
		var index = parseInt(rule.attr('data-id'));
		this.notes[index] = this.getValues(rule);
	...
	getValue: function (control) {
		...
		switch (type) {
			case 'username':
			case 'timestamp':
				result = parseInt(control.find('input[type="hidden"]').val());
				break;
...
MPHBAdmin.NumberCtrl = MPHBAdmin.Ctrl.extend({
	 ...
	 */
	getValue: function (control) {
		return control.children('input[type="number"]').val();
	},
	renderValue: function (control) {
...
MPHBAdmin.VariablePricingCtrl = MPHBAdmin.Ctrl.extend({}, {
	...
	pricesTable: null, // Top table (periods, base capacity, prices, extra guest prices)
	variationsTable: null, // Bottom table
	variationsTableBody: null,
	variationsTableFooter: null,

	// Place for new period inputs
	periodsRowEnd: null,

	// Place for new price inputs (in top table)
	basePriceRowEnd: null,
	extraAdultPricesRowEnd: null,
	extraChildPricesRowEnd: null,
	...
	init: function (element, args) {
		...
		this.name = element.attr('data-full-name');
		...
		this.pricesTable = element.children('.mphb-pricing-prices-table');
		...
		// Note that "td:last-child" is the actions column, not the last input
		this.periodsRowEnd = this.pricesTable.find('> tbody > tr:first-child > td:last-child');
		this.basePriceRowEnd = this.pricesTable.find('> tbody > tr.mphb-base-prices-row > td:last-child');
		this.extraAdultPricesRowEnd = this.pricesTable.find('> tbody > tr.mphb-extra-adult-prices-row > td:last-child');
		this.extraChildPricesRowEnd = this.pricesTable.find('> tbody > tr.mphb-extra-child-prices-row > td:last-child');
	...
	addPeriod: function () {
		...
		var periodInput = '<td data-period-index="' + index + '">'
			+ '<input type="number" name="' + this.name + '[periods][]" class="small-text" value="' + this.MIN_PERIOD + '" min="' + this.MIN_PERIOD + '" step="1" />'
			+ '<span class="mphb-pricing-tiny-description">' + this.periodDescription + '</span>'
			+ '<span class="dashicons dashicons-trash mphb-pricing-action mphb-pricing-remove-period" title="' + this.removePeriodText + '"></span>'
			+ '</td>';

		var pricesAtts = '';
		var afterPrice = '';

		var basePriceInput = '<td data-period-index="' + index + '">'
			+ '<input type="text" name="' + this.name + '[prices][]" class="mphb-price-text" value=""' + pricesAtts + ' />'
			+ afterPrice
			+ '</td>';

		var extraAdultPriceInput = '<td data-period-index="' + index + '">'
			+ '<input type="text" name="' + this.name + '[extra_adult_prices][]" class="mphb-price-text" value=""' + pricesAtts + ' />'
			+ afterPrice
			+ '</td>';

		var extraChildPriceInput = '<td data-period-index="' + index + '">'
			+ '<input type="text" name="' + this.name + '[extra_child_prices][]" class="mphb-price-text" value=""' + pricesAtts + ' />'
			+ afterPrice
			+ '</td>';

		var templateInput = '<td data-period-index="' + index + '">'
			+ '<input type="text" name="' + this.name + '[variations][$index$][prices][]" class="mphb-price-text" value=""' + pricesAtts + ' />'
			+ afterPrice
			+ '</td>';

		this.periodsRowEnd.before(periodInput);
		this.basePriceRowEnd.before(basePriceInput);
		this.extraAdultPricesRowEnd.before(extraAdultPriceInput);
		this.extraChildPricesRowEnd.before(extraChildPriceInput);
	...
	findLastPeriodIndex: function () {
		var periods = this.pricesTable.find('> tbody > tr:first-child > td[data-period-index]');
		var maxIndex = -1;
